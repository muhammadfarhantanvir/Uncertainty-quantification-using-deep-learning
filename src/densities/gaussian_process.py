...
from src.densities.random_fourier_features import RandomFourierFeatures
_SUPPORTED_LIKELIHOOD = ('binary_logistic', 'poisson', 'gaussian')
_SUPPORTED_RBF_KERNEL_TYPES = ['gaussian', 'laplacian']


def sample_orthogonal_matrix(shape, gain=1.0, generator:torch.Generator=None):
    ...
    a = torch.randn(flat_shape, generator=generator)
    ...
    return gain * torch.reshape(q, shape)

def orthogonal_random_features_initializer(weight, random_norm=True, stddev=1.0, generator:torch.Generator=None):
    """
    Initializes a weight tensor with orthogonal random features.
    """
    ...
    if num_rows < num_cols:
        ...
        while num_cols_sampled < num_cols:
            ortho_mat_square = sample_orthogonal_matrix(weight.shape, gain=stddev, generator=generator)
            ortho_mat_list.append(ortho_mat_square)
            num_cols_sampled += num_rows

        # Reshape the matrix to the target shape (num_rows, num_cols)
        ortho_mat = torch.concat(ortho_mat_list, dim=-1)
        ortho_mat = ortho_mat[:, :num_cols]
    else:
        ortho_mat = sample_orthogonal_matrix(weight.shape, gain=stddev, generator=generator)

    # Sample random feature norms.
    if random_norm:
        # Construct Monte-Carlo estimate of squared column norm of a random
        # Gaussian matrix.
        feature_norms_square = torch.randn(ortho_mat.shape, generator=generator)**2
    else:
        # Use mean of the squared column norm (i.e., E(z**2)=1) instead.
        feature_norms_square = torch.ones(shape=ortho_mat.shape)
...
class CustomRandomFeatureLayer(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(kernel_init(torch.empty(in_features, out_features)),
                                   requires_grad=False)
...
class RandomFeatureGaussianProcess(nn.Module):
    ...
    def __init__(
        ...
        normalize_input=False, #True in paper
        scale_random_features=True,
        return_random_features=False,
        return_covariance=True,
        kernel_type='gaussian',
        kernel_scale=1.0, # 2. in paper
        init_output_bias=0.0,
        kernel_scale_trainable=False,
        output_weight_trainable=False, #False in paper?
        output_bias_trainable=False, #True in paper
        use_custom_random_features=False, #True in paper
        covariance_momentum=0.999, #-1 in tf-code, but paper says 0.999
        covariance_ridge_penalty=1.0,
        covariance_likelihood='gaussian',
        custom_random_features_initializer=None, #OrthogonalRandomFeatures in paper
        ...
        self.output_weight_trainable = output_weight_trainable
        ...
        # Make random feature layer
            ...
            self.random_features = in_features
            random_features = in_features
        ...
        if return_covariance:
            if self.covariance_estimator is None:
                ...
                self.covariance_layer = self.covariance_estimator(
                    in_features=self.random_features,
                    momentum=covariance_momentum,
                    ridge_penalty=covariance_ridge_penalty,
                    likelihood=covariance_likelihood,
        ...
        self.output_layer.weight.requires_grad = self.output_weight_trainable # output bias should be only trainable param in GP, from paper
    ...
    def forward(self, x, last_epoch=None):
        ...
        if self.return_covariance:
            if last_epoch is not None:
                covariance = self.covariance_layer(Phi, logits, last_epoch=last_epoch).to(Phi.device)
            else:
...
class LaplaceRandomFeatureCovariance(nn.Module):
    ...
    def update_precision(self, Phi, logits):
        ...
        if self.likelihood == 'binary_logistic':
            ...
            prob_multiplier = torch.ones(1, device=Phi.device)
        else:
    ...
    def reset_precision(self):
        self.precision = self.init_precision.clone().to(self.precision.device)
...
class CrossEntropyRandomFeatureCovariance(LaplaceRandomFeatureCovariance):
    def __init__(self, *args, **kwargs):
        super(CrossEntropyRandomFeatureCovariance, self).__init__(*args, **kwargs)

    def forward(self, Phi, logits=None, last_epoch=True):
        # if not given weather last_epoch, make precision-matrix update every epoch
        ...
        """
        batch_size = Phi.shape[0]
        if self.training and last_epoch:
            self.update_precision(Phi=Phi, logits=logits)
            return torch.eye(batch_size, device=Phi.device)
        return self.compute_predictive_covariance(Phi=Phi)


def mean_field_logits(logits, covariance_matrix=None, mean_field_factor=25.): #factor was 1
    ...
    if covariance_matrix is None:
        variances = torch.ones(1, device=logits.device)
    else:
