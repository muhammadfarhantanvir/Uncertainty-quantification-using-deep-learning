...
class RandomFourierFeatures(nn.Module):
    ...
    def __init__(
        ...
        if kernel_type.lower() == 'gaussian':
            self.register_buffer('weight', torch.randn(in_features, out_features, generator=generator))
        elif kernel_type.lower() == 'laplacian':
            self.register_buffer('weight', torch.tan(math.pi * (torch.rand(in_features, out_features, generator=generator) - 0.5)))
        else:
            raise ValueError("Select kernel_type from one of ['gaussian', 'laplacian']")

        self.register_buffer('bias', torch.rand(out_features, generator=generator) * 2 * math.pi)
