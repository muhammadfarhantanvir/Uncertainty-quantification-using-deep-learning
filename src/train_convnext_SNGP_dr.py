...
# Configuration
num_classes = 5  # DR250 has 5 classes
batch_size = 64  
lr = 0.08
use_wd = False
wd = 3e-4
...
# Data block for dr250
data_path = Path(DATASET_DIRECTORY)/dataset
dr250 = get_data_block(dataset, train_labels_path=data_path/"trainLabels.csv", generator=tgen)
dls = dr250.dataloaders(data_path/"train", batch_size=batch_size, device=device)
...
neptune_model = "CS24-DRCNSG"

base_net = ConvNeXtSNGP(**net_params, generator=wgen).to(dls.device)
model = SNGP(
    **model_params,
...
items_test = dr250.get_items(data_path/"test")
